...
async function writeCollectionFile<K extends keyof ContactsState>(
  collection: K,
...
async function writeAllCollectionFiles(state: ContactsState) {
  ...
  await Promise.all(
    collections.map((collection) => writeCollectionFile(collection, state[collection]))
  );
}

async function readCollectionFile<K extends keyof ContactsState>(
  collection: K,
  fallback: ContactsState[K]
): Promise<ContactsState[K]> {
