  ...
  return participations.reduce<EventBalance>(
    (summary, participation) => {
      ...
      return {
        ...
        status: getAggregateStatus(roundMoney(summary.remaining + balance.remaining), roundMoney(summary.total + balance.total)),
      }
    },
...
export function getParticipationLabel(
  ...
  const eventSeries = occurrence ? series.find((item) => item.id === occurrence.seriesId) : undefined
  return {
