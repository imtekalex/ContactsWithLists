...
export const initialGroups: Group[] = [
  ...
  { id: "g3", name: "Technical Team", description: "Engineers & technical contacts", color: "purple" },
...
export const initialParticipations: EventParticipation[] = [
  {
    ...
    amountOwed: 420,
    currency: "EUR",
    notes: "Used early booking price and accidentally paid too much. The extra 30 EUR should show as credit.",
    payments: [
      {
...
export function resolveListMembers(
  ...
  return contacts.filter((c) => {
    ...
    if (f.advancedSearch && f.advancedSearch.query.trim() && f.advancedSearch.fieldKeys.length > 0) {
      if (!matchesAdvancedSearch(c, f.advancedSearch.query, f.advancedSearch.fieldKeys, customFields)) {
        return false
      }
    }
    return true
...
export function visibleCustomFieldsFor(
  contact: Contact,
  fields: CustomField[],
): CustomField[] {
