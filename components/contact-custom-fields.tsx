  ...
  if (!readOnly && allVisible.length === 0 && orphans.length === 0) {
    return (
      <p className="text-sm text-muted-foreground italic">
        No custom fields apply to this contact yet. Assign it to a group to unlock group-specific
        fields, or add a global custom field in Settings.
      </p>
    );
  }
      ...
      {orphans.length > 0 && (
        <div className="rounded-md border border-amber-200 bg-amber-50 p-3">
          ...
          <p className="text-xs text-amber-700 mt-0.5">
            These values were saved earlier but the field is no longer visible for this
            contact&apos;s current groups. They are kept so no data is lost.
          </p>
      ...
      return (
        <div className="grid grid-cols-1 gap-1 sm:grid-cols-[9rem_minmax(0,1fr)] sm:items-start">
          ...
          <Textarea
            value={v}
            onChange={(e) =>
              onChange(e.target.value ? { type: 'longText', value: e.target.value } : undefined)
            }
        ...
        <div className="grid grid-cols-1 gap-1 sm:grid-cols-[9rem_minmax(0,1fr)] sm:items-center">
          ...
          <Input
            ...
            onChange={(e) => {
              ...
              onChange(n === '' ? undefined : { type: 'number', value: Number(n) });
            }}
      ...
      return (
        <div className="grid grid-cols-1 gap-1 sm:grid-cols-[9rem_minmax(0,1fr)] sm:items-center">
          ...
          <Input
            ...
            value={v}
            onChange={(e) =>
              onChange(e.target.value ? { type: 'date', value: e.target.value } : undefined)
            }
            className="h-7 max-w-80 rounded-md border-0 bg-transparent px-1 shadow-none hover:bg-secondary/50 focus-visible:bg-background focus-visible:ring-1 focus-visible:ring-ring"
          />
        </div>
      ...
      return (
        <div className="grid grid-cols-1 gap-1 sm:grid-cols-[9rem_minmax(0,1fr)] sm:items-center">
          ...
          <select
            value={v}
            onChange={(e) =>
              onChange(e.target.value ? { type: 'dropdown', value: e.target.value } : undefined)
            }
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    ...
                    onChange(next.length > 0 ? { type: 'multiSelect', value: next } : undefined);
                  }}
                  className={cn(
...
export function hasCustomValuesToShow(contact: Contact, fields: CustomField[]): boolean {
  ...
  return fields.some((f) => !visibleIds.has(f.id) && hasValue(contact.customValues[f.id]));
}
